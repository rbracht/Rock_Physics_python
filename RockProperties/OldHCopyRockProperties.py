...
# Updated 10 March 2013 Don Easley on Mac at home
...
def dteSeventhModifiedHertzMindlinSpherePackEffectiveBulkModulus( newPressure,
	...
	assume a useful modified form would be ascaled version of the original, with
	...
	reference point (P0,K0). make the modification:
	...
	"""
	P  = newPressure * .001                                   # Conversion from Mpa to Gpa
	P0 = RefPressure * .001                                   # Conversion from Mpa to Gpa
	K0 = RefBulkModulus
	s = scale**expo

	Ap = K0 / (P0**expo)                                      # Compute alpha

	KhmMod = Ap * s * (P**expo - P0**expo) +  K1         # Compute modified Khm
	
	return KhmMod

def dteSeventhModifiedHertzMindlinSpherePackEffectiveShearModulus( KhmMod, MuRef, Kref, scale = 1.0 ):
	"""
	...
	if type( KhmMod ) == list:
		...
		MuHmMod = scale * (MuRef/Kref) * k**pow + (1.0-scale) * MuRef
...
def findHertzMindlinModuliFromReferenceState( Phi1, K0, Mu0, Phi0, Kmatrix, MuMatrix):
	"""
...
def dryFrameModuliChangeWithPressure( P1, P0, K0, Mu0, Phi0, Kmatrix, MuMatrix, scaleK=1.0, scaleMu=1.0 ):
	"""
	...
	state, favoured by the HM theory, is assumed to occure at porosity of 0.36 (V/V). Goth
	the predicted HS K and Mu's can be independently stretch squeezed but in practice it
	should only be the HS K's that is squeezed and use the resulting HS Mu's as is but
	it maybe necessary to fit the actual data you may have.

	(Input Variables)

	(Variable)    (Description)                                 (Unit)
	P1            New pressure to compute K1,Mu1                (Mpa)
	P0            effective pressure at reference state         (Mpa)
	K0            bulk modulus at reference state               (Gpa)
	Mu0           shear modulus at reference state              (Gpa)
	Phi0          porosity at reference state                   (V/V)
	Kmatrix       Matrix minerals net bulk modulus              (Gpa)
	MuMatrix      Matrix minerals net bulk modulus              (Gpa)
	scaleK        scaling factor to stretch/squeeze Khm
	scaleMu       scaling factor to stretch/squeeze MuHm
	...
	(Output Variables)
	K1            dry-frame bulk modulus at P1                  (Gpa)
	Mu1           dry-frame shear modulus at P1                 (Gpa)
	"""
	sK = scaleK
	sMu = scaleMu
	Phi1 = 0.36

	P1 = P1*0.001           # Conversion from Mpa to Gpa
	P0 = P0*0.001
	
	KhmP0, MuHmP0 = findHertzMindlinModuliFromReferenceState(Phi1, K0, Mu0, Phi0, Kmatrix, MuMatrix)

	KhmP1 = dteSeventhModifiedHertzMindlinSpherePackEffectiveBulkModulus( P1, P0, K0, sK )
	MuHmP1 = dteSeventhModifiedHertzMindlinSpherePackEffectiveShearModulus( Khm, K0, Mu0, sMu )

#	alpha = KhmP0 / (P0**(1.0/3.0))
#	beta = MuHmP0 / KhmP0
	
#	KhmP1 = s * alpha * P1**(1.0/3.0) + (1.0 - s) * K0
#	MuHmP1 = s * beta * KhmP1 + (1.0 - s) * Mu0
...
def gassmannModeling( initWaterSat, finalWaterSat, 
	...
	the variables with the prefix 'init' and a final state defined by variables prefixed with 
	...
	# Set the following variable to 1 if you want all intermediate results printed, 2 for partial and
	# anything else for no results printed.
	printVar = 0

	# I make the assumtion that Plith is constant so Peff and Ppore must be related
	# through the expression Peff = Plith - n*Ppore and n is often taken to be 1.0
	...
	if printVar == 1:
		...
		print "     Final dry fram bulk mod:    "+str(final_K_dry)+"  (Gpa)"
		print "     Initial dry fram shear mod: "+str(init_Mu_dry)+"  (Gpa)"
		print "     Final dry fram shear mod:   "+str(final_Mu_dry)+"  (Gpa)"
		print "     "
...
def linearFit( x, y ):
	...
	dimensionalnumerical python lists or numpy arrays (x,y) of equal
