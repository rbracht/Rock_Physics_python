...
"""
	def scaleALog( self, oldLogNum = int(-999), scalar = 1.0, newLogUnit = "" newLogId = ""):
		
		Creates a new log from an existing log scaled by user supplied scalar. 
		The user specifies the existing log by its number within the BunchOfLogs object.
		The first log by convention is numbered zero with subsiquent logs incremented by one.
		If no unit or id is supplied by the user the unit and id will be copied from the old
		log with "scaled" appended. If no scalar is supplied the new log will be equal to the old log

		if self.oldLogNum == -999:
			print "You have not specified a log number. The progam will now exit."
		if newLogUnit == "":
